...
/**
 * WordPress Plugin: EndlineX Class
 *
 * @author Patrik Vaďura
 */
class EndlineXCookie {

    public function __construct() {
        add_action('wp_footer', array($this, 'add_cookie_dialog'));
    }
    ...
    public function add_cookie_dialog() {
        ...
        echo '
            <div id="cookieModal" class="endlinex-backdrop"></div>
        
            <div id="cookieDialog" class="endlinex-cookie animate__animated animate__fadeIn">
                <div id="settingsDialog" class="endlinex-cookie-dialog pb-8 mb-8 border-b-[1px] border-gray-200">
                    <div class="title">'.$dialogSettingsTitle.'</div>    
                    <p>'.$dialogSettingsDescription.'</p> 
                    
                    <a href="' . esc_url($url) . '" class="show-more">' . __("Více o cookie", 'endlineX') . '</a>
                            ...
                            echo '
                                    <div x-data="{ open: false }" class="flex flex-col w-full">
                                        <button @click="open = ! open" class="flex flex-row justify-between items-center font-bold">
                                            ...
                                            <label for="' . $id .'" class="flex items-center cursor-pointer '. ($necessary ? "!opacity-50" : "") .'">
                                                <div class="relative cursor-pointer">
                                                  <input type="checkbox" id="' . $id .'" class="sr-only" '. ($necessary ? "checked disabled" : "") .'>
                ...
                <div class="endlinex-cookie-dialog">
                    <div class="title animate__animated animate__fadeIn">'.$dialogTitle.'</div>
